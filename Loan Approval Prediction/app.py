...
def predict():
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO emi_predictions
            (loan_amount, loan_months, interest_rate, emi, total_interest, total_payable, prediction_date)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """, (
            loan_amount, loan_months, interest_rate,
            emi, total_interest, total_payable, prediction_date
        ))
        conn.commit()
        cursor.close()
        conn.close()

        prediction_str = prediction_date.strftime("%Y-%m-%d %I:%M:%S %p")
        ...
        result_string = (
            ...
            f"Calculation made on: {prediction_str}"
        )
